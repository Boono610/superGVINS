...
cd ./catkin_GVINS/
catkin_make
source ./catkin_GVINS/devel/setup.bash
```
...
```
cd ./catkin_GVINS/src/GVINS/docker
make build
```
...
```
roslaunch gvins visensor_f9p_plnet.launch
```
Open another terminal and launch the rviz by:
```
rviz -d ./catkin_GVINS/src/GVINS/config/gvins_rviz_config.rviz
```
Then play the bag:
```
