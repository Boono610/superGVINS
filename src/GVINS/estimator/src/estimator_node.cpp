...
/**
 * @brief 从缓存中获取IMU、图像、GNSS数据，并对齐时间戳
 */
bool
getMeasurements(std::vector<sensor_msgs::ImuConstPtr> &imu_msg, sensor_msgs::PointCloudConstPtr &img_msg, std::vector<ObsPtr> &gnss_msg)
{
    ...
    // 对齐 IMU 和图像的时间戳
    // 检查 IMU 缓冲区中的最后一个时间戳是否大于图像时间戳，如果不是则返回 false ，等待 IMU 数据
    ...
    // 检查 IMU 缓冲区中的第一个时间戳是否大于图像时间戳，如果是，则丢弃图像缓冲区中的第一个图像
    ...
    if (GNSS_ENABLE)
    {
        // 检查 GNSS 缓冲区中的第一个 GNSS 数据的时间戳是否在图像时间戳的允许范围内
        // 如果 GNSS 数据的时间戳过早，则丢弃该 GNSS 数据； 如果 GNSS 缓冲区为空，则返回 false，表示需要等待更多 GNSS 数据； 否则存储到 gnss_msg
    ...
    // 从 IMU 缓冲区中获取所有时间戳小于或等于图像时间戳加上时间延迟 estimator_ptr->td 的 IMU 数据
...
void process()
{
    while (true)
    {
        ...
        // 遍历IMU数据，如果IMU时间戳小于或等于图像时间戳，则直接处理IMU数据;如果IMU时间戳大于图像时间戳，则对IMU数据进行插值，以匹配图像时间戳
        m_estimator.lock();
        ...
        for (auto &imu_data : imu_msg)
        {
            ...
            { 
                ...
                ROS_ASSERT(dt >= 0); // 保证 IMU 数据的时间戳是递增的
                current_time = t;
        ...
        TicToc t_s;
        // image用于存储图像数据。键是特征点的ID，值是一个向量，向量中的每个元素是一个pair，pair的第一个元素是相机ID，第二个元素是特征点的位置、像素坐标和速度
