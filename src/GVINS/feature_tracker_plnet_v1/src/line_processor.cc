...
/**
 * @brief 计算两个角度之间的差值，返回值为[0, pi]
 * @param angle1, angle2 输入的两个角度，单位为弧度
 */
...
/**
 * 将特征点分配到最近的线特征上,计算每个特征点到线的距离
 * @param relation 存储特征点到线特征的距离, key为特征点的索引，value为特征点到线的距离，单位为像素，vector的索引为线的索引
 */
...
/**
 * @brief 匹配线特征
 * @param line_matches 存储线特征匹配结果，line_matches[当前帧线特征索引] = 对应下一帧线特征索引，-1表示没有匹配
 */
void MatchLines(const std::vector<std::map<int, double>>& points_on_line0, 
  ...
  // 构建特征点与线特征的关联关系，为线特征索引，kv.first为点特征索引
