...
/**
 * 采用lightglue或superglue进行特征点匹配
 * @param matches 匹配结果，DMatch中的queryIdx和trainIdx分别表示第一幅图和第二幅图的特征点索引，distance表示匹配得分
 * @param outlier_rejection 是否进行外点剔除
 */
int PointMatcher::MatchingPoints(const Eigen::Matrix<float, 259, Eigen::Dynamic>& features0,
  ...
  if(_config.matcher == 0){ // lightglue
    // 采用lightglue进行特征点匹配
    Eigen::Matrix<int, Eigen::Dynamic, 2> matches_index; // 下标
    Eigen::Matrix<float, Eigen::Dynamic, 1> matches_score; // 匹配得分
    _lightglue->infer(normalized_features0.bottomRows(258), normalized_features1.bottomRows(258), matches_index, matches_score);
    // 将匹配结果转换为DMatch格式，如果需要外点剔除，则记录特征点坐标
    ...
    // 采用superglue进行特征点匹配
    // tip: 在img1中找img0的匹配，得到indices0和mscores0，反之得到indices1和mscores1
    Eigen::VectorXi indices0, indices1;  // 下标
    Eigen::VectorXd mscores0, mscores1;  // 匹配得分
