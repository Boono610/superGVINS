...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time)
{
    ...
    if (cur_pts.size() > 0)
    {
        ...
        vector<uchar> status;
        vector<float> err;
        cv::calcOpticalFlowPyrLK(cur_img, forw_img, cur_pts, forw_pts, status, err, cv::Size(21, 21), 3); // 跟踪特征点
    ...
    // 如果PUB_THIS_FRAME为真，删除异常点，设置掩膜，检测新的特征点，添加新的特征点
    if (PUB_THIS_FRAME)
    {
        // rejectWithF();
        ...
        if (n_max_cnt > 0)
        {
            ...
            */
            ...
            // std::cout << "Feature Detecting Done" << std::endl;

            // 根据匹配度排序，取前 MAX_CNT - forw_pts.size() 个特征点
            std::vector<std::pair<float, cv::Point2f>> features_sorted;
            for (int i = 0; i < features.cols(); i++)
            {
                cv::Point2f pt(features(1, i), features(2, i));
                float score = features(0, i);
                features_sorted.push_back(std::make_pair(score, pt));
            }
            std::sort(features_sorted.begin(), features_sorted.end(), [](const auto &a, const auto &b) {
                return a.first > b.first;
            });
            // std::cout << "Feature Sorting Done" << std::endl;

            n_pts.clear();
            int features_cnt = min(MAX_CNT - forw_pts.size(), features_sorted.size());
            // std::cout << "features_cnt: " << features_cnt << std::endl;
            for(int i = 0; i < features_cnt; i++)
            {
                cv::Point2f pt = features_sorted[i].second;
                // std::cout << "feature " << i << ": " << pt.x << ", " << pt.y << std::endl;
                n_pts.push_back(pt);
            }
            // std::cout << "Feature Selecting Done" << std::endl;
        }
        else
            n_pts.clear();
