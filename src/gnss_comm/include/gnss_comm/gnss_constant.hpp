...
namespace gnss_comm
{
    ...
    struct EphemBase
    {
        ...
        gtime_t  ttr;                   /* transmission time in GPST */ //GPST中的传输时间
        gtime_t  toe;                   /* ephemeris reference time in GPST */ //GPST中的星历参考时间
        uint32_t health;                /* satellite health */ //卫星健康
        double   ura;                   /* satellite signal accuracy */ //卫星信号精度
        uint32_t iode;
    };
    ...
    struct Obs                                           /* observation data record */
    {
        ...
        uint32_t    sat;                                 /* satellite number */ //卫星编号
        std::vector<double> freqs;                       /* received satellite frequencies */ //接收到的卫星频率
        std::vector<double> CN0;                         /* signal strength */ //信号强度
        std::vector<uint8_t> LLI;                        /* signal lost-lock indictor */ //信号丢失锁定指示器
        std::vector<uint8_t> code;                       /* code indicator (CODE_???) */ //掩码指示器
        std::vector<double> psr;                         /* observation data pseudorange (m) */ //观测数据伪距
        std::vector<double> psr_std;                     /* pseudorange std (m) */ //伪距标准差
        std::vector<double> cp;                          /* observation data carrier-phase (cycle) */ //观测数据载波相位
        std::vector<double> cp_std;                      /* carrier-phase std (cycle) */ //载波相位标准差
        std::vector<double> dopp;                        /* observation data doppler frequency (Hz) */ //观测数据多普勒频率
        std::vector<double> dopp_std;                    /* doppler std (Hz) */ //多普勒标准差
        std::vector<uint8_t> status;                     /* cycle slip valid flag. bit_0 (psr valid), bit_1(cp valid), bit_2(half cp valid, *ublox), bit_3(half cp subtracted, *ublox) */ 
                                                            //循环滑动有效标志，bit_0（psr有效），bit_1（cp有效），bit_2（一半cp有效，*ublox），bit_3（一半cp减去，*ublox）
    };
